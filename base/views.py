...
# デコレーターにより、CreatePostViewへのアクセスはログインユーザーに限定される
# ログイン状態でなければsettings.pyのLOGIN_URLにリダイレクトされる
@method_decorator(login_required, name='dispatch')
class CreatePostView(CreateView):
    '''投稿ページのビュー
    
    PostFormで定義されているモデルとフィールドと連携して
    投稿データをデータベースに登録する
    ...
    '''
    # forms.pyのPostFormをフォームクラスとして登録
    form_class = PostForm
    # レンダリングするテンプレート
    template_name = "post_content.html"
    # フォームデータ登録完了後のリダイレクト先
    success_url = reverse_lazy('base:post_done')
    ...
    def form_valid(self, form):
        ...
        parameters:
          form(django.forms.Form):
            form_classに格納されているPostFormオブジェクト
        Return:
...
class DeleteView(DeleteView):
    ...
    # 操作の対象はPostモデル
    model = Post
    # post_delete.htmlをレンダリングする
    template_name ='post_delete.html'
    # 処理完了後にマイページにリダイレクト
    success_url = reverse_lazy('base:mypage')

    def delete(self, request, *args, **kwargs):
      '''レコードの削除を行う
      
      Parameters:
        self: DeleteViewオブジェクト
...
def contact_view(request):
    ...
    #return render(request, "'BASE_DIR/', program/learning.html", {'form': form})